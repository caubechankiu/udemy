...
import { setUser, setGetMyCourses, addViewCourse, showModal, changeWishlist, takeCourse } from '../actions'
...
class ModalPurchase extends React.Component {
    ...
    onClickPay(courseid) {
        if (this.props.courseCost > this.props.creditBalance) {
            this.setState({ message: 'The credit balance is not enough to make payments' })
        ...
        return (
            <Modal show={this.state.show} onHide={this.onHide.bind(this)}>
                <Modal.Header closeButton>
                    <Modal.Title className="text-center h3">Confirm purchase!</Modal.Title>
                </Modal.Header>
                <Modal.Body style={{ overflow: 'auto' }}>
                    ...
                    <div className='row'>
                        <div className='col-xs-8'>
                            <h4><strong>{this.props.courseName}</strong></h4>
                        </div>
                        <div className='col-xs-4 text-right'>
                            <h4><strong>{this.props.courseCost == 0 ? 'Free' : (this.props.courseCost + '$')}</strong></h4>
                        </div>
                    </div>
                    <div className='row'>
                        <div className='col-xs-8'> <h4><strong>Credit Balance</strong></h4></div>
                        <div className='col-xs-4 text-right'><h4><strong>{this.props.creditBalance}$</strong></h4></div>
                    </div>
                    <hr style={{ borderColor: 'silver' }} />
                    <div className='row'>
                        <div className='col-xs-8'><h4><strong>You pay</strong></h4></div>
                        <div className='col-xs-4 text-right'><h4><strong>{this.props.courseCost}$</strong></h4></div>
                    </div>
                </Modal.Body>
                <Modal.Footer>
...
class ViewCourse extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.course) {
            this.getCoursesRelateLecture(nextProps.course.lecturer._id, nextProps.params.id)
    ...
    render() {
        if (!this.props.course) {
            ...
            let previewvideo = <video id={"video-" + this.props.params.id} className="col-md-8 col-xs-12 video-js" controls preload="false"
                width="100%" height="auto" style={{ paddingLeft: '0px' }}>
            ...
            let controls = <div className='col-md-4 col-xs-12'>
                ...
                <h5>
                    Instructed by : <Link to={'/view-user/' + this.props.course.lecturer._id} >
                        <strong>{this.props.course.lecturer.username}</strong>
                    </Link>
                </h5>
                <h5>
                    Category : <Link to={'/courses/' + this.props.course.genre._id}>
                        <strong>{this.props.course.genre.name}</strong>
                    </Link>
                    {' / '}
                    <Link to={'/courses/' + this.props.course.genre._id + '/' + this.props.course.subgenre._id}>
            ...
            return (
                <div>
                    <div className='container'>
                        <h1><em style={{ color: 'teal' }}>
                            {this.props.course.name}
                        </em></h1>
                        <Glyphicon glyph='calendar' />
                        <span>{' Created ' + new Date(this.props.course.createdAt).toLocaleString()}</span>
                        <div>
                            <span className="course-rate">
                                <span style={{ width: (this.props.course.star || 0) * 20 + '%' }}></span>
                            </span>
                            <span style={{ fontWeight: 'bold' }} >
                                {' ' + (this.props.course.star ? this.props.course.star.toFixed(1) : 0) + '(' + this.props.course.numberofreviews +
                                    ' ratings) • ' + this.props.course.numberofstudent + ' students enrolled'}
                            </span>
                        </div>
                    ...
                    <div className='container'>
                        <div className='row' style={{ marginTop: '20px' }}>
                            <Col md={8} xs={12} style={{ paddingLeft: '0px' }}>
                                <div className="panel panel-primary">
                                    <div className="panel-heading">
                                        <h1 id='About' className="panel-title">About This Course</h1>
                                    </div>
                                    <div className="panel-body">
                                        <strong style={{ color: '#5bc0de' }}>Course Description</strong>
                                        <div dangerouslySetInnerHTML={{ __html: this.props.course.description }} style={{ fontSize: '16px', marginTop: '10px' }} />
                                        ...
                                        <div style={{ fontSize: '16px', marginTop: '10px' }} >
                                            <ul>
                                                {this.props.course.targetstudent.map((item, index) => {
                                                    return <li key={index}>{item}</li>
                                                })}
                                            </ul>
                                        </div>
                                ...
                                <div className="panel panel-primary">
                                    <div className="panel-heading">
                                        <h1 id='Lectures' className="panel-title">Lectures</h1>
                                    </div>
                                    <div className="panel-body">
                                        {this.props.course.lectures.map((lecture, index) => {
                                            return <div key={index} style={{ fontSize: '16px' }}>
                                                ...
                                                {this.props.course.lectures.length != index + 1 ? <hr style={{ borderColor: 'silver' }} /> : ''}
                                            </div>
                                        })}
                                    </div>
                                </div>
                            ...
                            <Col md={4} smHidden xsHidden>
                                <Row>
                                    <Col xs={12}>
                                        <p><strong>{'Courses taught by '}
                                            <Link to={'/view-user/' + this.props.course.lecturer._id}>{this.props.course.lecturer.username}</Link>
                                        </strong></p>
                                    </Col>
                        ...
                        <div className="panel panel-primary">
                            <div className="panel-heading">
                                <h1 id='Instructor' className="panel-title">Instructor Biography</h1>
                            </div>
                            <div className="panel-body">
                                <div className='row'>
                                    <div className='col-xs-2 col-md-1' style={{ paddingLeft: '0px', paddingRight: '0px' }}>
                                        <div className="photo-profile" style={{ backgroundImage: 'url(' + ('/api/resource/images?src=' + this.props.course.lecturer.photo + '&w=100&h=100') + ')' }}></div>
                                    </div>
                                    <div className='col-xs-10 col-md-11' >
                                        <Link to={'/view-user/' + this.props.course.lecturer._id} className='lecturer-name'>{this.props.course.lecturer.username}</Link>
                                        <br className='hidden-xs' />
                                        {this.props.course.lecturer.twitter ? <a target='_blank' href={'https://twitter.com/' + this.props.course.lecturer.twitter}>
                                            <img className='icon-social' src='/images/Twitter-icon.png'></img>
                                        </a> : ''}
                                        {this.props.course.lecturer.youtube ? <a target='_blank' href={'https://www.youtube.com/' + this.props.course.lecturer.youtube}>
                                            <img className='icon-social' src='/images/YouTube-icon.png'></img>
                                        </a> : ''}
                                        {this.props.course.lecturer.linkedin ? <a target='_blank' href={'https://www.linkedin.com/' + this.props.course.lecturer.linkedin}>
                                            <img className='icon-social' src='/images/linkedin-icon.png'></img>
                                        </a> : ''}
                                        {this.props.course.lecturer.website ? <a target='_blank' href={this.props.course.lecturer.website}>
                                            <img className='icon-social' src='/images/browser-icon.png'></img>
                                        </a> : ''}
                                        {this.props.course.lecturer.googleid ? <a target='_blank' href={'https://plus.google.com/' + this.props.course.lecturer.googleid}>
                                            <img className='icon-social' src='/images/google-plus-icon.png'></img>
                                        </a> : ''}
                                        {this.props.course.lecturer.facebookid ? <a target='_blank' href={'https://facebook.com/' + this.props.course.lecturer.facebookid}>
                                            <img className='icon-social' src='/images/facebook-icon.png'></img>
                                        </a> : ''}
                                    </div>
                                </div>
                                <div dangerouslySetInnerHTML={{ __html: this.props.course.lecturer.biography }} style={{ fontSize: '16px', marginTop: '10px' }} />
                        ...
                        <div className="panel panel-primary">
                            <div className="panel-heading">
                                <h1 id='Reviews' className="panel-title">Reviews</h1>
                            </div>
                            <div className="panel-body">
                                <div className='row'>
                    ...
                    <ModalPurchase ref='modalPurchase' courseName={this.props.course.name} courseId={this.props.params.id}
                        courseCost={this.props.course.cost} creditBalance={this.props.creditBalance}
                        onTakeCourseSuccess={() => {
